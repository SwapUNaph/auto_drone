...
class bebop_data:
    def __init__(self):        
        ...
        self.vichile_pub = rospy.Publisher(     "/rviz/vehicle",       MarkerArray,            queue_size=1, latch=True)
        self.gate_pub = rospy.Publisher(        "/rviz/gate",          MarkerArray,            queue_size=1, latch=True)
        self.arena_pub = rospy.Publisher(       "/rviz/arena",         MarkerArray,            queue_size=1, latch=True)
        self.fly_pub = rospy.Publisher(         "/rviz/fly",           MarkerArray,            queue_size=1, latch=True)
        self.look_pub = rospy.Publisher(        "/rviz/look",          MarkerArray,            queue_size=1, latch=True)
        ...
        rospy.Subscriber("/bebop/reset", Empty, self.callback_reset)
        rospy.Subscriber("/bebop/odom", Odometry, self.callback_odom)
        rospy.Subscriber("/bebop/cmd_vel", Twist, self.callback_cmds)
        rospy.Subscriber("/bebop/states/common/CommonState/BatteryStateChanged", CommonCommonStateBatteryStateChanged,self.callback_battery)
        rospy.Subscriber("/bebop/states/common/CommonState/WifiSignalChanged", CommonCommonStateWifiSignalChanged,self.callback_wifi)

        
        rospy.Subscriber("/auto/wp_fly", WP_Msg, self.callback_wp_fly)
        rospy.Subscriber("/auto/wp_look", WP_Msg, self.callback_wp_look)
        

        rospy.Subscriber("/auto/state_auto", Int32, self.callback_state)
        rospy.Subscriber("/auto/pose", Drone_Pose, self.callback_vehicle_pos)
        
        rospy.Subscriber("/auto/filtered_gate_WP", WP_Msg, self.callback_gate_filtered)
    ...
    def callback_gate_filtered(self,data):
        ...
        point_marker = Marker()
        point_marker.header.frame_id = 'gate_estimate_frame'
        point_marker.header.stamp    = rospy.get_rostime()
        point_marker.ns = "gate_estimate_frame"
        point_marker.id = 0
        point_marker.type = 1
        point_marker.action = 0
        point_marker.scale.x = .25
        point_marker.scale.y = .25
        point_marker.scale.z = .25
        ...
        self.tbr.sendTransform((data.pos.x, data.pos.y, data.pos.z),(0,0,0,1),rospy.get_rostime(),'gate_estimate_frame', "vehicle_frame")
        self.vichile_pub.publish(marker_array)
        
        self.gate_pub.publish(marker_array)
        

    def callback_vehicle_pos(self,data):
    ...
    def callback_wp_look(self,data):
